...
#   PUSH             push to ECR (default: true; set false for a local-only
#                    --load build that does not touch AWS)
#   ECR_PUSH_LATEST  also tag/push :latest (default: false; PUSH=true only)
set -euo pipefail
...
[[ "$(uname -s)" == "Linux" ]] \
  || die "this script targets Linux hosts; on macOS build the full Dockerfile with: docker buildx build --platform linux/arm64 -f Dockerfile ."
...
if [[ "$PUSH" == "true" ]]; then
  ...
  IMAGE_URI="${IMAGE_URI:-${APP_NAME:-gitmirrorcache}:${IMAGE_TAG}}"
fi
