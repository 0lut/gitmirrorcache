...
if [ "$family" != "$expected_family" ]; then
  echo "refusing to optimize: expected ECS task family $expected_family, got $family" >&2
  exit 10
fi
if [ "$container_name" != "$expected_container" ]; then
  echo "refusing to optimize: expected ECS container $expected_container, got $container_name" >&2
  exit 11
fi

if [ -d "$repo_dir" ]; then
  echo '--- before count-objects ---'
  docker exec "$container_id" git -C "$repo_dir" count-objects -vH || true
fi

echo '--- optimize ---'
docker exec "$container_id" /usr/local/bin/git-cache optimize "$repo"
