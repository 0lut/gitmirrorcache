...
async fn upload_pack_large_repo() {
    ...
    let advertised = git
        ...
        .await
        .expect("advertise refs for large repo");
    let advertised_text = String::from_utf8_lossy(&advertised.stdout);
    assert!(
        advertised_text.contains("refs/cache/main"),
        "should advertise refs/cache/main"
    );

    let request = format!("0032want {head_sha}\n00000009done\n");
    let response = git
        .upload_pack_stateless_rpc(&cache_repo, request.as_bytes(), 1024, 16 * 1024 * 1024)
        .await
        .expect("stateless rpc for large repo");
